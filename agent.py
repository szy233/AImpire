...
TOOL_CACHE_TTL = 30          # 只读工具结果缓存秒数

# 触发注入状态上下文的关键词（其余消息跳过 summary_text() 节省 token）
_STATE_KEYWORDS = {"状态", "gpu", "任务", "实验", "进程", "查看", "检查", "日志", "训练", "跑", "运行"}

# 只读工具（结果可安全缓存）
_READONLY_TOOLS = {"check_task_status", "git_info", "scan_projects",
                   "get_project_config", "experiment_history", "list_files"}

# 触发快速模型的关键词（无需复杂推理的状态查询）
_FAST_KEYWORDS = {"看", "查", "状态", "进度", "有没有", "多少", "几个", "列出", "显示", "是否", "好不", "如何了"}
_COMPLEX_KEYWORDS = {"修改", "创建", "生成", "写", "改", "跑", "运行", "同步", "提交", "分析", "优化", "新建", "帮我", "执行"}
...
def _pick_model(cfg, user_message: str) -> str:
    """简单状态查询用 fast_model（haiku），复杂任务用主模型（sonnet）。"""
    fast = getattr(cfg, 'fast_model', None)
    if not fast:
        return cfg.model
    msg = user_message.strip()
    if len(msg) > 120 or '\n' in msg:
        return cfg.model
    if any(w in msg for w in _COMPLEX_KEYWORDS):
        return cfg.model
    if any(w in msg for w in _FAST_KEYWORDS):
        return fast
    return cfg.model
...
class Agent:
    def __init__(self):
        ...
        # tool_cache: (name, args_key) -> (result_str, expire_timestamp)
    ...
    async def process_message(
        ...
        selected_model = _pick_model(cfg, user_message)
        use_thinking = THINKING_BUDGET > 0 and _supports_thinking(selected_model)
        api_extra = {}
        if use_thinking:
    ...
    def _execute_tool(self, name: str, params: Dict[str, Any]) -> str:
        ...
        result_str = self._run_tool(name, params)

        if cache_key:
            self._tool_cache[cache_key] = (result_str, _time.time() + TOOL_CACHE_TTL)
        return result_str
